...
void Composite::Add(std::unique_ptr<StatsAggregator> aggr) { aggregators.push_back(std::move(aggr)); }
...
void Min::PrintValue(std::ostream& out) const { out << "Min is " << current_min; }
...
void Max::PrintValue(std::ostream& out) const { out << "Max is " << current_max; }
