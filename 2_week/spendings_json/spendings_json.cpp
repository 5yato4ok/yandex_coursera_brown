...
vector<Spending> LoadFromJson(istream& input) {
  auto doc = Load(input);
  vector<Spending> res;
  for (auto& val : doc.GetRoot().AsArray()) {
    auto& map_val = val.AsMap();
    res.push_back({map_val.at("category").AsString(), map_val.at("amount").AsInt()});
  }
  return res;
}
...
void TestLoadFromJson() {
  ...
  const vector<Spending> expected = {{"food", 2500}, {"transport", 1150}, {"restaurants", 5780}, {"clothes", 7500}, {"travel", 23740}, {"sport", 12000}};
