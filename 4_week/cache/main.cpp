...
void TestCachingPtr(const Library& lib) {
  auto unpacker = make_shared<BooksUnpacker>();
  ICache::Settings settings;
  settings.max_memory = INT32_MAX;
  auto cache = MakeCache(unpacker, settings);

  auto ptr1 = cache->GetBook(lib.book_names[0]);
  cache->GetBook(lib.book_names[1]);
  auto ptr2 = cache->GetBook(lib.book_names[0]);
  auto ptr3 = cache->GetBook(lib.book_names[2]);
  ASSERT_EQUAL(ptr1, ptr2);
  auto ptr4 = cache->GetBook(lib.book_names[2]);
...
#define RUN_CACHE_TEST(tr, f) tr.RunTest([&lib] { f(lib); }, #f)
  ...
  RUN_CACHE_TEST(tr, TestCachingPtr);
