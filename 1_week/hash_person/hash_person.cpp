...
struct AddressHasher {
  size_t operator()(const Address& val) const {
    int x = 2'946'901;
    auto a = hash_int(val.building);
    auto b = hash_str(val.city);
    auto c = hash_str(val.street);
    return (a * x * x + b * x + c);
  }
...
struct PersonHasher {
  size_t operator()(const Person& val) const {
    int x = 2'946'901;
    auto a = hash_str(val.name);
    auto b = hash_int(val.height);
    auto c = hash_double(val.weight);
    auto d = AddressHasher{}(val.address);
