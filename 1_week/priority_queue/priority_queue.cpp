...
class PriorityCollection {
 public:
  ...
  bool IsValid(Id id) const { return data.find(id) != data.end(); }

  // Получить объект по идентификатору
  const T& Get(Id id) const { return data.at(id).first; }
  ...
  pair<const T&, int> GetMax() const {
    return {data.at(prev(priority.end())->second).first,
            prev(priority.end())->first};
  }
  ...
  pair<T, int> PopMax() {
    ...
    return make_pair(std::move(max_data.mapped().first),
                     max_data.mapped().second);
  }
