...
StatsData BuildStatsData(vector<Person> people) {
  ...
  {
    sort(people.begin(), people.end(), [](const Person& lhs, const Person& rhs) { return lhs.income > rhs.income; });
...
int main() {
  ...
  for (string command; cin >> command;) {
    if (command == "AGE") {
      ...
      cout << "There are " << std::distance(adult_begin, end(stats.sorted_by_age)) << " adult people for maturity age "
           << adult_age << '\n';
      ...
      char gender;
      cin >> gender;
      const auto& most_popular_name = gender == 'M' ? stats.most_popular_male_name : stats.most_popular_female_name;
