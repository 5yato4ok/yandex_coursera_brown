...
class IndexProxy {
 public:
  ...
  {
    if (pos_ < coeffs_.size()) val = coeffs_[pos_];
    val = 0;
  }
  ...
  void operator=(const T& val)  // setter
  {
    if (pos_ >= coeffs_.size()) {
      if (val != 0) {
        if (pos_ >= coeffs_.size()) coeffs_.resize(pos_ + 1);
        coeffs_[pos_] = val;
      }
    } else {
      coeffs_[pos_] = val;
...
bool operator!=(const IndexProxy<T>& index, const T& other) {
  if (index.pos_ >= index.coeffs_.size()) return 0 != other;
...
class Polynomial {
 ...
 public:
  ...
  IndexProxy<T> operator[](size_t pos) { return IndexProxy<T>(coeffs_, pos); }
