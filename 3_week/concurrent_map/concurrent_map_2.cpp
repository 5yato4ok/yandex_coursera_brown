...
class ConcurrentMap {
 public:
  ...
  using StorageBucketType = std::vector<Bucket>;

  //Could be made as child of lock_guard and stores the data inside
  // as argument to the const send the bucket
  struct WriteAccess {
    ~WriteAccess() { m.unlock(); };
    V& ref_to_value;
    mutex& m;
  };
  ...
  explicit ConcurrentMap(size_t bucket_count) : data(StorageBucketType(bucket_count)) {}

  WriteAccess operator[](const K& key) {
    auto index = GetIndex(key);
    data[index].m.lock();
    return {data[index].data[key], data[index].m};
  }
  ...
  ReadAccess At(const K& key) const {
    ...
    try {
      data[index].m.lock();
      return {data[index].data.at(key), data[index].m};
  ...
  bool Has(const K& key) const {
    auto index = GetIndex(key);
    data[index].m.lock();
    bool result = data[index].data.find(key) != data[index].data.end();
    data[index].m.unlock();
    return result;
  }
  ...
  MapType BuildOrdinaryMap() const {
    ...
    for (int i = 0; i < data.size(); i++) {
      data[i].m.lock();
      for (const auto& pair_ : data[i].data) {
        result.insert(pair_);
      }
      data[i].m.unlock();
    }
 ...
 private:
  size_t GetIndex(const K& key) const { return hasher(key) % data.size(); }
  StorageBucketType data;
  //StorageDataType data;
  //mutable StorageMutexType mutexs;
  Hash hasher;
};
